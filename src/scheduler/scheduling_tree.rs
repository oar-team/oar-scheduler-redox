...
pub fn schedule_jobs_ct(slot_sets: &mut HashMap<String, TreeSlotSet>, waiting_jobs: &mut Vec<Job>, hierarchy: &Hierarchy) {
    waiting_jobs.into_iter().for_each(|job| {
        ...
        assign_resources_mld_job_split_slots(slot_set, job, hierarchy);
    });
...
pub fn assign_resources_mld_job_split_slots(slot_set: &mut TreeSlotSet, job: &mut Job, hierarchy: &Hierarchy) {
    ...
    job.moldables.iter().enumerate().for_each(|(i, moldable)| {
        if let Some((tree_node, proc_set)) = slot_set.find_node_for_moldable(moldable, hierarchy) {
